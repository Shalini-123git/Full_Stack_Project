...
router.get("/report", restrictTo("doctor"), async (req, res) => {
    const reports = await Report.find({});
    res.render("reports/index.ejs", {reports});
})

router.get("/medicalHistory", restrictTo("doctor"), async (req, res) => {
    const medicalHistories = await MedicalHistory.find({});
    res.render("medicalHistory/index.ejs", {medicalHistories});
})

router.get("/timeline", restrictTo("doctor"), async (req, res) => {
    const timeline = await Timeline.find({});
    res.render("timeline/index.ejs", {timeline});
})

router.get("/checklist", restrictTo("doctor"), async (req, res) => {
    const checklist = await Checklist.find({});
    res.status(200).json({message: checklist});
})

router.get("/appointments", restrictTo("doctor"), async (req, res) => {
